                ...
                <div class="row breadcrumbs-top">
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="{{route('brands.index')}}"> {{trans('admin.brands')}}
                                </a>
                            </li>
                            <li class="breadcrumb-item active"> {{trans('admin.edit')}} - {{$brand -> name}}
                            </li>
                        </ol>
                    </div>
                </div>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title" id="basic-layout-form"> {{trans('admin.edit')}} - {{$brand -> name}} </h4>
                                            ...
                                            <div class="tab-content">
                                                ...
                                                <div class="tab-pane fade show @if($key == 0) active @endif" id="{{$locale}}" role="tabpanel">
                                                    <div class="col form-group">
                                                        ...
                                                        <input type="text" name="{{ $locale.'[name]' }}" id="{{ $locale . '[name]' }}" value="{{old($locale.'.name',$brand->translate($locale)->name )}}" placeholder="@lang('admin.name')" class="form-control @error(" $locale.name" ) is-invalid @enderror" value="{{ old($locale.'.name') }}">
                                                        @error("$locale.name" )
                                                ...
                                                @if($brand -> _parent)
                                                <div class="row">
                                                    <div class="col-md-12" id="main_brand">
                                                        <div class="form-group ">
                                                            <label for="parent_id"> {{trans('admin.select_brand')}} </label>
                                                            <select name="parent_id" class="form-control" id="parent_id">
                                                                <option value="" disabled selected>{{trans('admin.select_brand')}}</option>
                                                                @if($brands && $brands -> count() > 0)
                                                                @foreach($brands as $mainbrand)
                                                                <option value="{{$mainbrand -> id }}" @if($mainbrand -> id == $brand -> parent_id) selected @endif >{{$mainbrand -> name}}</option>
                                                                @endforeach
                                                                @endif
                                                            </select>
                                                            @error('parent_id')
                                                ...
                                                <div class="col form-group">
                                                    ...
                                                    <p><img style="width:200px;height:200px" src="{{asset($brand->logo)}}"></p>
